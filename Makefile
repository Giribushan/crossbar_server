...
crossbar:
	docker run \
		--rm -d -p 8080:8080 --name crossbar \
		crossbario/crossbar
...
python:
	docker run \
		-it --rm --link crossbar \
		-v C:/Users/gcheru200/git/autobahn-java/demo-gallery/python:/test  \
		crossbario/autobahn-python \
		python -u /test/test_component2.py
...
java:
	docker run \
		-it --rm --link crossbar \
		-v C:/Users/gcheru200/git/autobahn-java:/workspace \
		crossbario/autobahn-java:netty \
			/bin/bash -c "gradle installDist -PbuildPlatform=netty && DEMO_GALLERY_OPTS="-DlogLevel=INFO" \
			demo-gallery/build/install/demo-gallery/bin/demo-gallery ws://crossbar:8080/ws"
